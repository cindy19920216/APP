...
const getMarketData = unstable_cache(
  async () => {
    const SYMBOLS = [
      '^GSPC', '^IXIC', '^KS11', '^KQ11', '^N225', '^GDAXI', '^HSI',
...
export async function GET() {
  try {
    ...
    const commodities = [
      ...
      { name: '원유(WTI)', symbol: 'CL=F',    icon: 'ti-droplet-filled',   bg: '#1a1a2a', ic: '#7F77DD', prefix: '$' },
      { name: '은',        symbol: 'SI=F',    icon: 'ti-medal',            bg: '#1e2626', ic: '#5DCAA5', prefix: '$' },
      { name: 'BTC',       symbol: 'BTC-USD', icon: 'ti-currency-bitcoin', bg: '#251a10', ic: '#EF9F27', prefix: '$' },
